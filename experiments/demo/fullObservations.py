...
# Resolve workspace root from the script location and make it the cwd
...
# Pass fpath to the constructor so postProcessing uses the session folder
# from the start (it cleans up fpath on __init__, so the folder must exist first)
...
samples.ForwardProblemHMC(
    ...
    path=os.path.join(SESSION_DIR, 'f.pt'),
)
...
samples.InverseProblemHMC(
    ...
    path=os.path.join(SESSION_DIR, 'Mi.pt'),
)

# ── Plots (saved to SESSION_DIR/figs/) ────────────────────────────────────────
post.plotForwardPaper(data_path=os.path.join(SESSION_DIR, 'f.pt'), name='fullObs')
post.plotInversePaper(data_path=os.path.join(SESSION_DIR, 'Mi.pt'), name='fullObs')
