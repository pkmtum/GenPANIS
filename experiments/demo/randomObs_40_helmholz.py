...
# ── Random observation mask (40 points) ───────────────────────────────────────
observationGrid = torch.load(os.path.join(SESSION_DIR, 'ind40.pt'),
                             map_location=device)

# ── Inverse problem HMC (Helmholtz, 40 random observations) ──────────────────
samples.InverseProblemHMC(
    ...
    observationGrid=observationGrid, pde='helmholz',
    path=os.path.join(SESSION_DIR, 'randomObs_40_helmholz.pt'),
)

# ── Plot ──────────────────────────────────────────────────────────────────────
post.plotInversePaper(
    data_path=os.path.join(SESSION_DIR, 'randomObs_40_helmholz.pt'), name='randomObs_40_helmholz'
)
