...
# Override Darcy defaults from input.py before constructing pde
...
parser = argparse.ArgumentParser(description='Train genPANIS on Helmholtz (10 000 labeled samples)')
...
parser.add_argument('--cuda_index', type=int, default=1,
                    help='CUDA device index to train on (default: 1)')
...
# ── PDE (Helmholtz: BC = 1., pde_type = 'helmholz') ─────────────────────────
...
# ── Virtual-observables dataset: all 20 000 Helmholtz samples ────────────────
dxyvo = (torch.load('./data/labeledData20000Helm_129.pth', map_location=device)
         .unsqueeze(-3).flatten(-2)[:, :20000, :, :].to(dtype=torch.float32))
...
model_name      = "genPANIS_helmholz_10000"
...
if args.resume:
    print(f"Resuming Helmholtz training from: {args.resume}")
    trainer.fit(samples, train_loader, ckpt_path=args.resume)
else:
    print("Starting Helmholtz training from scratch...")
    trainer.fit(samples, train_loader)

print("Helmholtz training finished.")
