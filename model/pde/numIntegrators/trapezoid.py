...
def trapzInt2DParallelnorm(f_xy):
    out = torch.trapezoid(f_xy, torch.linspace(-1, 1, f_xy.size(dim=-1)), dim=-1)
    return torch.trapezoid(out, torch.linspace(-1, 1, f_xy.size(dim=-2)), dim=-1)

def trapzInt2DParallelnormFDM(f_xy):
    out = torch.trapezoid(f_xy, torch.linspace(-1, 1, f_xy.size(dim=-1)+2)[1:-1], dim=-1)
    return torch.trapezoid(out, torch.linspace(-1, 1, f_xy.size(dim=-2)+2)[1:-1], dim=-1)
