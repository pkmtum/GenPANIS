...
class rbfInterpolation:
    def __init__(self, grid, gridW, gridW2, sgrid, tau, tauW, rhs=None, uBc=None, reducedDim=None, savePath=None, options=None, pde_type='darcy'):
        """
        ...
        self.parallelLocalMassMatrix = torch.func.vmap(self.localMassMatrix)
        self.parallelLocalMassTensor = torch.func.vmap(self.localMassTensor)
        ...
        self.m = self.parallelLocalMassMatrix(torch.stack(self.rnodeList))
        self.mT = self.parallelLocalMassTensor(torch.stack(self.rnodeList))
    ...
    def findRbfCoeffs(self, closedForm=None, y=None, N=None):
        if closedForm is None or closedForm == False:
            if y == None:
                phi = None
            else:
            ...
            A = self.shapeFunc.view(self.shapeFunc.size(0), -1).t()
            #phi = torch.einsum('...k,kj->...j', y.view(y.size(0), -1), (torch.inverse(A.t() @ A) @ A.t()))
            phi = torch.einsum('...j,ij->...i', y.flatten(-2), (torch.inverse(A.t() @ A) @ A.t()))
            print('Max Absolute Error of RBF fitting: ', (self.cTrialSolutionParallel(phi)-y).abs().max())
            print('Mean Absolute Error of RBF fitting: ', (self.cTrialSolutionParallel(phi)-y).abs().mean())
        return phi
        #return phi.detach()
    ...
    def solveCGPDE(self, c_x, f=None, uBc=None):
        ...
        for i in range(0, 1): 
            ...
            #et = torch.exp((self.padBCs(u)-self.u0PDE)*self.alphaPDE) # For Non-Linear PDE
            et = None # For Linear PDE
            ...
            if torch.max(torch.abs(torch.einsum('...ij,...j->...i', K, u) - B)) < 10**(-4): ## Impossible to get when using single precision so I use -4 in that case
                notConverged = False
                break
            else:
                notConverged = True
    ...
    def solveCGPDE_helmholz(self, c_x, f=None, uBc=None):
        ### Initialization
        f = self.rhs
        if c_x.dim() == 2:
            u = torch.zeros(self.nonBcNodes.size())
        ...
        for i in range(0, 1): 
            ...
            #et = torch.exp((self.padBCs(u)-self.u0PDE)*self.alphaPDE) # For Non-Linear PDE
            et = None # For Linear PDE
            K = self.assembleStiffMatrixHelmholz(c_x)
            M = self.assembleMassMatrixHelmholz(k=1., c_x=c_x)
            Kt = -K + M
            K, rhsFromDirichlet = self.applyDirichletConditions(Kt, uBc=uBc)
            dRdYinv = torch.linalg.inv(K)
            #### For the source term the c_x is averaged over each element to avoid any integration with quadrature. This introduces an error that we condider managable in this context.
            #B = self.assembleSourceTermHelmholz(c_x=c_x)
            #B = + B[..., self.nonBcNodes] - rhsFromDirichlet
            B = - rhsFromDirichlet
    ...
    def solveCGPDE_dispatch(self, c_x, f=None, uBc=None):
        if self.pde_type == 'helmholz':
            return self.solveCGPDE_helmholz(c_x=1/c_x**1.30103, f=f, uBc=uBc)
        else:
            return self.solveCGPDE(c_x=c_x, f=f, uBc=uBc)

    def solveCGPDELinear(self, c_x, f=None, uBc=None):
        f = self.rhs
        K, dummy, rhsFromDirichlet = self.assembleStiffMatrix(c_x, uBc)
    ...
    def localMassMatrix(self, elemNodes):
        """
        :param x: x coordinate of the point
        :param y: y coordinate of the point
        :return: The local triangular basis functions
        ...
        JacDeterm = self.dl**2 # = 2*A
        Area = JacDeterm/2.
        m = (torch.diag(torch.ones(3)) + torch.ones(3, 3))*Area/12
        
        return m
    
    def localMassTensor(self, elemNodes):
        """
        :param x: x coordinate of the point
        :param y: y coordinate of the point
        :return: The local triangular basis functions
        ...
        for m in range(3):
            for i in range(3):
                for j in range(3):
                    unique = len(set([m, i, j]))
                    if unique == 1:       # m = i = j
                        T[m, i, j] = Area / 10.0
                    elif unique == 2:     # two indices equal
                        T[m, i, j] = Area / 30.0
                    else:                 # all indices different
                        T[m, i, j] = Area / 60.0
    ...
    def assembleStiffMatrixHelmholz(self, c_x):


        k = self.k.expand(*c_x.size()[:-2], *self.k.size())
        Kp = torch.zeros(self.reducedDim**4, *c_x.size()[:-2])
    ...
    def applyDirichletConditions(self, K, uBc=None):
        Kt = K[..., self.nonBcNodes, :]
    ...
    def assembleMassMatrixHelmholz(self, k, c_x, uBc=None, et=None, ut=None):

        c_x = c_x.view(*c_x.size()[:-2], -1)
        c_x = c_x[..., self.rnodesMap]
        #c_x = torch.mean(c_x, dim=-1).view(*c_x.size()[:-2], -1, 1).repeat(*(1 for _ in range(c_x.dim() - 1)), 3) # For averaging over all the conductivities of the element
        ...
        #m = torch.einsum('...ij,ijk->...ijk', c_x, self.m)
        m = torch.einsum('...il,ijkl->...ijk', c_x, self.mT)


        #m = k**2 * self.m
        #m = m.expand(*c_x.size()[:-2], *m.size())
        Mp = torch.zeros(self.reducedDim**4, *c_x.size()[:-2])
        m = m.reshape(*m.size()[:-3], -1).t()
        Mp.index_add_(0, self.rkind[3], m)
        Mp = Mp.t().view(*c_x.size()[:-2], self.reducedDim**2, self.reducedDim**2)  
    ...
    def assembleSourceTermHelmholz(self, c_x):

        c_x = c_x.view(*c_x.size()[:-2], -1)
        c_x = c_x[..., self.rnodesMap]
        #c_x = torch.mean(c_x, dim=-1).view(*c_x.size()[:-2], -1, 1).repeat(*(1 for _ in range(c_x.dim() - 1)), 3)

        #b = torch.einsum('...ij,ij->...ij', c_x, self.bb) ### This is use when I take the mean and I do a rough approximation
        b = torch.einsum('...ik,ijk->...ij', c_x, self.m) ### Even though confusing at first, it is the result of interpolating c_x and doing quadrature to find b.


        Bp = torch.zeros(self.reducedDim**2, *c_x.size()[:-2])

        b = b.reshape(*c_x.size()[:-2], -1).t()
        index = self.rnodesMap.flatten()
        Bp.index_add_(0, index, b)
        Bp = Bp.t().view(*c_x.size()[:-2], self.reducedDim**2)  
